...
* Why use this malloc?
  ...
  * For very large requests (>= 128KB by default), it relies on system
    ...
    0                  64                                                                                   512                  128K~   (bytes)
    +-------------------+-------------------------------------------------------------------------------------+---------------------+--------------------
    | caching allocator |                                                                                     | best fit allocator  |           mmap-->
    +-------------------+-------------------------------------------------------------------------------------+---------------------+--------------------
...
* Vital statistics:
  ...
  Supported pointer representation:       4 or 8 bytes // 支持32 bit & 64 bit 指针
  Supported size_t  representation:       4 or 8 bytes // 在64 bit 系统下也支持 32 bit 的size_t,可手动将size_t设置为64
       Note that size_t is allowed to be 4 bytes even if pointers are 8.
       You can adjust this by defining INTERNAL_SIZE_T

  Alignment:                              2 * sizeof(size_t) (default)
       MALLOC_ALIGNMENT默认按2倍的size_t对齐,也可以手动设置使其超出实际需要
       (i.e., 8 byte alignment with 4byte size_t). This suffices for
       nearly all current machines and C compilers. However, you can
       define MALLOC_ALIGNMENT to be wider than this if necessary.

  Minimum overhead per allocated chunk:   4 or 8 bytes
       为了管理被分配的内存,需要一些额外的bit来存储管理信息(当前chunk的大小以及状态信息)
       最小为4 bytes 或者8 bytes,这取决于指针位数, 32 bit 指针使用 4 bytes 管理chunk
       64 bit 指针可以使用 4 bytes也可以使用 8 bytes 管理chunk
       Each malloced chunk has a hidden word of overhead holding size
       and status information.

  Minimum allocated size: 4-byte ptrs:  16 bytes    (including 4 overhead)
			  8-byte ptrs:  24/32 bytes (including, 4/8 overhead)
        32位系统使用16 bytes作为chunk的最小值,12 bytes 的 payload + 4 bytes的管理开销,即实际申请小于此值也将按此大小分配chunk
        64位系统使用24/32 bytes作为chunk的最小值,20 bytes payload + 4 bytes/24 bytes payload + 8 bytes
       ...
       16/24/32 bytes.
       已分配chunk的最小尺寸之所以为16/24/32是因为其payload尺寸12/20/24正好为未分配chunk的size
       payload加上4/8的overhead正好可以表示已分配chunk的size,此时未分配chunk整体将成为已分配chunk的payload
       未分配chunk因为size和status都可以用0表示所以无overhead
       未分配chunk的payload为4+8 4+16 8+16 三种情况
       - 4+8
         4 bytes 用于追踪size,8 bytes用于持有空闲列表的指针
       - 4+16
       - 8+16

         +--------+---------------------------------------------------+--------+
         | size_t |                  payload                          | size_t |
         +--------+---------------------------------------------------+--------+

       Even a request for zero bytes (i.e., malloc(0)) returns a
       pointer to something of the minimum allocatable size.
       即使要求分配size=0的内存也会返回最小尺寸的内存空间
       ...
       我们知道,mmap()是分配虚拟内存页的,1 page = 4kb,如果一次映射4kb+1b的文件也需要2页即8kb的映射,这类文件非常多的话,浪费物理内存是很明显的
       Linux的做法是准备1～2页的remainder,按页分配虚拟内存但是不按页映射物理内存,每次映射超出文件size部分但是不满1页的多余部分可以映射到物理内存的相同位置
       在本例中这种做法可以节省2*(4kb-1b)的物理内存
  ...
  Maximum allocated size:  4-byte size_t: 2^32 minus about two pages
       ...
       limitation.
       chunk的寻址范围完全取决于size_t的大小,4bytes的size_t理论上可以寻址2^32,signed的情况下寻址范围2^31,8bytes同理
       
       Requests for sizes outside the allowed range will perform an optional
       failure action and then return null. (Requests may also
       fail because a system is out of memory.)
...
#if USE_TCACHE
/*tunables是glibc的一种功能,允许通过设置的环境变量来改变runtime library的行为,比如GLIBC_TUNABLES=glibc.malloc.trim_threshold=128:glibc.malloc.check=3*/
...
/*
  ...
  通常过小的chunk会先记录当达到一定尺寸统一清理,通过将TRIM_FASTBINS标志置1采用激进回收策略
  该策略在程序中有大量小尺寸chunk时会极大的影响速度
*/
...
/* Some systems support the concept of tagging (sometimes known as
   ...
   很多系统支持内存染色和指针染色技术,当指针被解引用时会和指向的内存比色,颜色不一致时可能会禁止访问
   ...
   我们在维护malloc数据结构的时,将每一个夹杂用户数据的chunk分发出去时都会为其打上单色
   这样在simple buffer泛滥时可以更快速的被定位,这样以下的情况可以被避免:
    一个被分配的指针所指向的内存已被释放之后又被分配给其他指针时
    用旧指针引用新数据的user-after-free错误就可以被检测
   ...
   对于支持染色的系统分配一块全0内存区域使用tag_new_zero_region,而对于不支持染色的系统则简单调用memset将所有位置0 
...
/*
  ...
  分配n大小的内存chunk,如果n=0则返回最小chunk 16/24/32
  当系统无足够内存分配额时返回null
  n支持的最大值通常随系统而变化,不过一定小于size_t类型的最大值
*/
...
/*
  ...
  返回指向n大小chunk的指针,如果空间不足则返回null

  The returned pointer may or may not be the same as p. The algorithm
  prefers extending p when possible, otherwise it employs the
  equivalent of a malloc-copy-free sequence.
  返回的指针可能与原指针相同,虽然系统倾向于返回一个增长的指针
...
/*
  ...
  return 0.
  内部通过给sbrk接收负数来下移堆指针紧缩堆空间以回收高层空闲chunk并将资源返还操作系统
  注意此函数仅返回最上层空闲chunk,如果最上层为已分配chunk
  则夹在中间的空闲chunk无法释放,成功释放则返回1否则0,在sbrk不支持负数的系统上永远返回0
  接受的参数非返还OS的空闲chunk尺寸而是期望保留空闲chunk尺寸,如果该参数为0,则除了维持数据结构的必要开销以外全部返还OS
  保留的空闲chunk唯一服务于未来的堆内存分配,而不需要再重新向OS申请物理内存
*/
...
/*
  ...
  返回给定指针指向的chunk中用户数据空间的尺寸,实际尺寸可能会大于用户申请尺寸
  因为最小chunk尺寸不能低于16/24/32的最低限度,或者用户申请非对齐尺寸,则可用数据为对齐尺寸
...
/*
  ...
  由于维护数据结构需要额外的开销,即使用户未分配空间,当前使用空间也会大于0
  mallinfo可以获取更多的信息,如果在malloc外单独调用sbrk调整了堆则malloc_stats获取的数据可能不准确
*/
...
/*
  ...
  可以通过键值对的方式设置环境参数改变malloc的行为
  * M_MXFAST->定义fastbins小块内存的阈值,小于该阈值的小块内存不会尝试合并,当请求相同大小的chunk是可被复用,可以提高访问速度,但是产生大量小碎片
  * M_MMAP_THRESHOLD->设置阈值,用户请求大于阈值时使用mmap()分配内存,默认为128kb,0则表示不支持mmap(),所有内存分配均通过brk调节堆指针
  * M_TOP_PAD->每次调用sbrk都会以此参数为基准预留一段空间,防止频繁的malloc导致频繁的系统调用,在一些sbrk执行缓慢的系统上这个参数非常有用,第一次sbrk预留较大的pad可以影响第二次malloc不至于再触发系统调用
  * M_MMAP_MAX->mmap()函数在同一时间内最多处理的用户请求数量,一般置0表示禁用mmap()
  * M_TRIM_THRESHOLD->
  *
*/
...
/*
  ...
  超过M_MMAP_THRESHOLD设定的阈值则使用mmap()分配内存即使存在空闲chunk,除非整块空闲chunk可以容纳请求尺寸则使用chunk
  ...
  使用mmap()来映射大块内存,malloc映射小块请求,mmap()映射的内存块会独立堆获取与释放,且mmap()分配到的内存空间无法重用
  ...
  mmap区别于chunk单独映射的好处:
   1. Mapped区域可以单独释放返还OS,保证了在持续运行的项目中,OS端物理内存占用维持在一个比较低的水平
   2. Mapped区域不会出现allocated chunk[freed chunk]allocated chunk导致freed chunk无法释放的情况
   3. 在一些存在内存空洞的系统上,mmap()可以获取brk无法获取到的内存
  ...
  mmap区别于chunk单独映射的缺点:
   1. 不具有chunk的灵活性,如不能置为空闲chunk为后续请求再利用,或者和已有chunk合并成更大的chunk为未来的大请求做准备
   2. 相比与chunk对内存的浪费更明显,因为mmap()申请的内存必须按页分配
   3. 通常映射一块内存,malloc要比mmap效率高

  The advantages of mmap nearly always outweigh disadvantages for
  "large" chunks, but the value of "large" varies across systems.  The
  default is an empirically derived value that works well in most
  systems.
  虽然chunk分配速度优于malloc,但是大chunk在后续带来的副作用使得分配大块内存时mmap()成为更优的选择
  ...
  现如今,brk()和mmap()已没有明确界限,并且共用一块区域,以往将128kb作为mmap调用的界限也不再适合
  ...
  to zero.
  如今mmap的开销依然很高,每当kernel获取一段物理内存映射都需要将其置0再交予应用空间
  置0操作不仅会消耗缓存还会降低内存的带宽
  ...
  在从前的32bit x86系统,brk最大不能超过8MB,否则将hit mmap,如今的VA设计下mmap和brk都可以完全独享heap区域
  ...
  在128Kb的位置放置一块滑块,在64bit系统上最大滑动到32Mb
  ...
  在程序初期大量mmap分配内存,之后一部分被回收,回收后的空间可供brk后续使用,通过移动滑块使用超过128Kb的空间
  ...
  estimator预测当有固定大小的空间被mmap释放时,紧接着会分配一个等大小的空间在brk,所以将会上移该大小为第三次请求做准备
...
/*
  INTERNAL_SIZE_T:同size_t,可设置,当前机器值为8 bytes,等同于指针宽度
*/
...
/*
   malloc_chunk details:
    ...
    chunk-> +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
	    ...
	    +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
	    |             Size of chunk, in bytes                     |A|M|P| AMP可以和size存储在一起是因为,chunk是8/16字节对齐的即0000 1000/0001 0000表示8/16的倍数所以最后3～4bits永远为0
      mem-> +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
...
nextchunk-> +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
      ...
      这里有一个trick就是,当前chunk的头部为上一个chunk的size|当前chunk的size(overhead)这正好也是上一个chunk的尾部即当前chunk的数据size|下一个chunk的size(overhead)
    ...
    chunk经常在字边界开始,应用数据区也是在字边界开始,所以chunk至少是双字对齐的
...
nextchunk-> +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    P位置1表示前一个chunk已被使用,如果当前chunk的P位置1,则试图pointer-word来访问上一个chunk的size将会发生异常
    P位置0表示前一个chunk未被使用,可以通过pointer-word来访问上一个chunk的size,也可以通过对size的运算来计算上一个chunk的指针
    ...
    该位置1表示当前chunk在secondary arena
    该位置0表示当前chunk在main arena
    当free函数获得一个指针时,会通过指针定位到A值,判定A值使用不同策略来释放内存
    当A=1时,表示当前chunk由子线程创建,在释放时,会遍历所有arena的,来寻找chunk所在的subheap
    当A=0时,表示当前chunk由主线程创建,在释放时,只需要在main arena中做一次简短的循环来找到initial aerna
	...
	hooks.c.
  当M=1时可以忽略P和A,表示当前chunk是由mmap分配的,未来由OS单独释放,既不需要判断P(上一个chunk的状态,为合并chunk做准备),也不需要判断A,因为mmap不存在arena

     3. Chunks in fastbins are treated as allocated chunks from the
	point of view of the chunk allocator.  They are consolidated
	with their neighbors only in bulk, in malloc_consolidate.
  Fastbin中的chunk被当作已分配chunk处理,只有当大量存在时才会调用malloc_consolidated进行合并
*/
...
/* Conversion from malloc headers to user pointers, and back.  When
   ...
   malloc head -> user pointer & user pointer -> malloc head的转换
   可能会包含从其他地址取址并生成一个可供使用的指针
...
#define CHUNK_HDR_SZ (2 * SIZE_SZ)// chunk header size 16 bytes
...
/* 通过chunk size + 16 bytes定位到mem address作为参数传给tag_at()函数获得一个tag指针*/
...
/* Check if REQ overflows when padded and aligned and if the resulting value
   ...
   check fail.  
   PTRDIFF_MAX = 0x7FFF FFFF FFFF FFFF*/
static inline bool
...
/* size field is or'ed with PREV_INUSE when previous adjacent chunk in use 
   如果前一个chunk正在使用,则OR操作0x1用于将P置位1*/
...
/*
   ...
   int malloc优先使用已有用户空间内存,用户空间内存不足时调用sysmalloc向内核请求分配内存
 */
...
static void *sysmalloc (INTERNAL_SIZE_T nb, mstate av)
{
  ...
  size_t pagesize = GLRO (dl_pagesize);// 4096
   ...
   */
  // arena为空 或 大于使用mmap分配内存的最小阈值且小于最大mmap分配chunk数则使用mmap分配
  // mmap用于初始化arena或分配大于阈值的内存
  if (av == NULL || ((unsigned long) (nb) >= (unsigned long) (mp_.mmap_threshold) && (mp_.n_mmaps < mp_.n_mmaps_max)))
    ...
    try_mmap:
      /*
         ...
         因为mmaped chunk不需要保存prev size,所以当前chunk可以使用下一个chunk的prevsize,因此需要的开销仅为一个SIZE_T

         See the front_misalign handling below, for glibc there is no
         need for further alignments unless we have have high alignment.
         mmaped chunk不同于freed chunk 和 allocated chunk有两个SIZE-SZ的overhead,仅有一个SIZE_SZ的overhead,因为其无需标记上一个chunk的size,故节省一个SIZE_SZ的开销
       */
...
//    |                                             V                                                |                                                                                                                               V
//+---+----+-----------+------+---------------+-----+-------+-------+------------------+----------+--+--+----------------+------+--------+------+-----------+------------------+---------+-------------+--------------------------------------------------------------+
//| ar_ptr | pre| heap | size | mprotect size | pad | mutex | flags | ha|e fast chunks | fastbins | top | last remainder | bins | binmap | next | next free | attached threads | sys mem | max sys mem |                            TOP                               |
//+--------+-----------+------+---------------+-------------+-------+------------------+----------+--+--+----------------+------+--------+------+-----------+------------------+---------+-------------+--------------------------------------------------------------+
//                                                  ^                                                |
//                                                  |                                                |
//                                                  |                                                |
...
//    |                                                                                                                                      V
//+---+----+-----------+------+---------------+-----+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
//| ar_ptr | pre| heap | size | mprotect size | pad |                                                                                       TOP                                                                                                                       |
//+--------+-----------+------+---------------+-----+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
static void tcache_thread_shutdown (void)// 释放tcache结构体本身以及所关联的chunk
{
  int i;
  ...
  /* Free all of the entries and the tcache itself back to the arena
     heap for coalescing.  */
  for (i = 0; i < TCACHE_MAX_BINS; ++i)// 释放所有entry
    {
      while (tcache_tmp->entries[i])// 释放entry中所有chunk
	      {
...
static void
tcache_init(void)// 调用int_malloc分配一块sizeof(tcache_perthread_struct)大小的内存,并清零供tcache使用
{
  mstate ar_ptr;
  void *victim = 0;
  ...
  if (!victim && ar_ptr != NULL)//arena已初始化但是内存分配失败则retry
    {
...
// MAYBE_INIT_TCACHE会综合考虑当前tcache状态以及系统是否允许使用tcache来决定是否初始化tcache
  ...
  for (;; )
    {
      ...
      while ((victim = unsorted_chunks (av)->bk) != unsorted_chunks (av))
        {
          bck = victim->bk;// bck为最后一块unsorted bin中的chunk,且unsorted bin非空
          ...
          /*
             ...
             满足small bin size但是因为其他原因导致未从small bin中分配,则尝试从last remainder中切割
           */
          ...
          /* remove from unsorted list取出unsorted bin中最后的chunk*/
          if (__glibc_unlikely (bck->fd != victim))
          ...
          /* place chunk in bin 如果从unsorted bin中取出的chunk满足small bin size*/
          ...
          if ((victim = first (bin)) != bin && (unsigned long) chunksize_nomask (victim) >= (unsigned long) (nb))
            {
              ...
              /* Avoid removing the first entry for a size so that the skip
                 ...
                 通常相同size的chunk并列时,fd/bk_nextsize存储在第一个chunk中,所以当发生此情况时,直接使用下一个chunk,避免多余的指针操作*/
              ...
              /* Exhaust */
              if (remainder_size < MINSIZE)// 如果剩余chunk不足以再次分配,则包括不足部分分配大于请求size的chunk
                {
                  set_inuse_bit_at_offset (victim, size);
                  if (av != &main_arena)
		                set_non_main_arena (victim);
      ...
      ++idx;
      bin = bin_at (av, idx);
      block = idx2block (idx);// block为当前bin所属binmap的索引,binmap共4个,每个binmap有32位分别保存32个bin的标记
      map = av->binmap[block];// 通过索引获得bin所在的binmap
      bit = idx2bit (idx);// 获取bin在binmap中32个bit所处的位置

      for (;; )// 遍历map中的bit直到值为1即对应的bin含有freed chunk
        {
          /* Skip rest of block if there are no more set bits in this block.  */
          if (bit > map || bit == 0)// bit>map表示当前map中无空闲chunk
            {
              do
                {
              ...
              while ((map = av->binmap[block]) == 0);// 如果当前map为0,即map中32个bin不包含空闲chunk,则循环处理下一个map
             
              bin = bin_at (av, (block << BINMAPSHIFT));// 在含空闲chunk的map中从头开始遍历bin
              bit = 1;
            }

          /* Advance to bin with set bit. There must be one. */
          while ((bit & map) == 0)// map中bit位置的bin不含有空闲chunk,直到找到含有空闲chunk的bin时退出,先前已经验证过map不为0,即map中必有bit位为1
            {
              bin = next_bin (bin);
              bit <<= 1;
              assert (bit != 0);
          ...
          if (victim == bin)
            ...
            {
              size = chunksize (victim);// 此时victim为size对应large bin中下一个bin的最后一个即bin中最小chunk
        ...
        // large bin不存在可用freed chunk则考虑切分Top Chunk
				use_top:
					/*
					...
					if ((unsigned long) (size) >= (unsigned long) (nb + MINSIZE))// 如果Top Chunk Size足够分配则切割,remainder作为新Top
						{
							remainder_size = size - nb;
							remainder = chunk_at_offset (victim, nb);
							av->top = remainder;
             ...
             Top不足以分配但是有Fast Bin Chunk,尝试释放fast chunk,之后使用small bin或large bin来分配*/
             ...
             Top Chunk不足以分配,且无fast bin可回收,系统调用来扩充Top
					 */
					else
						{
							void *p = sysmalloc (nb, av);
